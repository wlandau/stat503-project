...
PISA math and reading scores naturally divide USA subjects into high-performing students (top 25\%) and low-performing students (bottom 25\%). I will select a subset of variables in the student-specific PISA data and attempt to predict student performance (high or low) in the United States.


# Exploratory analysis

The student-specific USA data has roughly five hundred variables for predicting reading and math scores. After removing the few continuous variables and the variables with all missing values, I am left with 256 categorical variables for prediction. As seen in Figure \ref{fig:missing}, most variables still have a large fraction of missing values.
...
To explore the data further and  256 variables and build a classifier, I rank the variables according to a simple matching heuristic. The heuristic is similar to the simple matching coefficient between two categorical variables, and it loosely measures how well a factor accurately splits students according to high or low performance. For each factor $x$, I calculate this version of the matching coefficient as follows.
