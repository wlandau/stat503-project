...
- K nearest neighbors with $K$ = 5, 10, and 25 (``class`` package in R [@class]).
...
I would like to thank Dr. Cook for steering me in the right direction. The PISA data is cumbersome, and the guidance is very appreciated. In addition to core R [@R], I used the packages ``class`` [@class], ``DMwR`` [@DMwR], ``e1071`` [@e1071], ``gdata`` [@gdata], ``ggplot2`` [@ggplot2], ``gridExtra`` [@gridExtra], ``knitr`` [@knitr], ``MASS`` [@MASS], ``nnet`` [@nnet], ``plyr`` [@plyr], ``randomForest`` [@randomForest], and ``reshape2`` [@reshape2].
